...
static void redirect_io(int input, int output, t_data *data)
{
	...
	if (input < 0 || output < 0)
		cleanup_n_exit(1, data); // or use a specific error code/message

	// Redirect standard input to 'input'
	if (dup2(input, STDIN_FILENO) == -1)
		cleanup_n_exit(1, data); // Specific error for input redirection failure


	// Redirect standard output to 'output'
	if (dup2(output, STDOUT_FILENO) == -1)
		cleanup_n_exit(1, data); // Specific error for output redirection failure
}
 ...
 *   - Executes the command using execve. If execve fails, handles the error using cleanup_n_exit.
 *
 * Notes:
 ...
 * is invalid (less than 0), the function will call `cleanup_n_exit` to handle the situation. Similarly,
...
static void execute_child_process(t_data *data)
{
	...
	if (data->cmd_options == NULL || data->cmd_path == NULL) {
		cleanup_n_exit(ERROR, data); // Consider using a specific error message or code
	}
	// Execute the command
